...
function processFor (el) {
  if (el.tag === 'import') {
    ...
    getAndRemoveAttr(el, 'zf:for')
    getAndRemoveAttr(el, 'zf:for-items')
    return
  }
  let exp =
    getAndRemoveAttr(el, 'wx:for') || getAndRemoveAttr(el, 'wx:for-items') || getAndRemoveAttr(el, 'zf:for') || getAndRemoveAttr(el, 'zf:for-items')
  if (exp) {
    const inMatch = exp.match(tplBracket)
    if (!inMatch) {
      process.env.NODE_ENV !== 'production' &&
        warn(`Invalid zf:for expression: ${exp}`)
      return
    }

    el.for = exp
    pushProp(exp)
    if ((exp = getAndRemoveAttr(el, 'wx:for-item')) || (exp = getAndRemoveAttr(el, 'zf:for-item'))) {
      el.alias = exp
      pushProp(exp)
    } else {
      el.alias = 'item'
    }
    if ((exp = getAndRemoveAttr(el, 'wx:for-index')) || (exp = getAndRemoveAttr(el, 'zf:for-index'))) {
      el.iterator1 = exp
      pushProp(exp)
    } else {
      el.iterator1 = 'index'
    }
    if ((exp = getAndRemoveAttr(el, 'wx:key')) || (exp = getAndRemoveAttr(el, 'zf:key'))) {
      el.key = exp
      pushProp(exp)
    } else {
      el.key = ''
...
function processIf (el) {
  const exp = getAndRemoveAttr(el, 'wx:if') || getAndRemoveAttr(el, 'zf:if')
  if (exp) {
    const inMatch = exp.match(tplBracket)
    if (!inMatch) {
      process.env.NODE_ENV !== 'production' &&
        warn(`Invalid zf:if expression: ${exp}`)
      return
    }
    ...
    if (getAndRemoveAttr(el, 'zf:else') != null) {
      el.else = true
    }
    const elseif = getAndRemoveAttr(el, 'wx:elif') || getAndRemoveAttr(el, 'zf:elif')
    if (elseif) {
      const inMatch = elseif.match(tplBracket)
      if (!inMatch) {
        process.env.NODE_ENV !== 'production' &&
          warn(`Invalid zf:if expression: ${elseif}`)
        return
      }
