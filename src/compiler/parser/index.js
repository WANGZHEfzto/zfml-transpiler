...
function processFor (el) {
  ...
  let exp =
    getAndRemoveAttr(el, 'zf:for') || getAndRemoveAttr(el, 'zf:for-items')
  if (exp) {
    ...
    el.for = exp
    pushProp(exp)
    if ((exp = getAndRemoveAttr(el, 'zf:for-item'))) {
      el.alias = exp
      pushProp(exp)
    } else {
      el.alias = 'item'
    }
    if ((exp = getAndRemoveAttr(el, 'zf:for-index'))) {
      el.iterator1 = exp
      pushProp(exp)
    } else {
      el.iterator1 = 'index'
    }
    if ((exp = getAndRemoveAttr(el, 'zf:key'))) {
      el.key = exp
      pushProp(exp)
    } else {
      el.key = ''
...
function processIf (el) {
  const exp = getAndRemoveAttr(el, 'zf:if')
  if (exp) {
    const inMatch = exp.match(tplBracket)
    if (!inMatch) {
      process.env.NODE_ENV !== 'production' &&
    ...
    const elseif = getAndRemoveAttr(el, 'zf:elif')
    if (elseif) {
